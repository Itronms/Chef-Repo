...
$VC = Read-Host -Prompt "Which Data Center are you running?(TP,LL,TX)"

if ($VC -eq 'TP'){$Connection = "esxtpll-vc01.itronms.local"}
elseif ($VC -eq 'LL'){$Connection = "itron-p-vm-vc.itronhosting.local"} 
elseif ($VC -eq 'TX'){$Connection = "esxtx-vc01.itronms.local"}
...
foreach($cluster in Get-Cluster){
    ...
    @{N="Number of VM's";E={($esx | Measure-Object -InputObject {$_.Extensiondata.Vm.Count} -Sum).Sum}}  | convertto-html -head $a -body $Title | Out-file C:\Temp\Clusterspec.html -Append


}
Invoke-Expression C:\Temp\Clusterspec.html