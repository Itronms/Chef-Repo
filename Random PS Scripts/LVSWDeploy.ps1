...
$script:FN = Read-Host -Prompt "Is this an FN Build? (Yes or No)"
...
if ($script:Cluster -eq 'P') {$TargetCluster = Get-Cluster -Name "LV-Windows";$DomainControllerVMName = "$script:customer-P-DC01";$DC02VMName = "$script:customer-P-DC02"}

if ($script:Cluster -eq 'NP') {$TargetCluster = Get-Cluster -Name "LV-Windows";$DomainControllerVMName = "$script:customer-NP-DC01";$DC02VMName = "$script:customer-NP-DC02"}

$RHCluster = "LV-Linux"
$UIP = ($script:customernumber - 1500)
$scriptApp = ($script:customernumber)
$scriptDB = ([Int]$script:customernumber + 1000)
$scriptDMZ = ([Int]$script:customernumber + 2000)
...
$Referencegroup= Get-VDPortgroup -Name "104-CyberArk"

New-VDPortgroup -VDSwitch LV-Production -Name vLAN-$scriptApp-$NP-App -ReferencePortgroup $Referencegroup | Set-VDPortgroup -VlanId $scriptApp

New-VDPortgroup -VDSwitch LV-Production -Name vLAN-$scriptDB-$NP-DB -ReferencePortgroup $Referencegroup | Set-VDPortgroup -VlanId $scriptDB -NumPorts 8

New-VDPortgroup -VDSwitch LV-Production -Name vLAN-$scriptDMZ-$NP-DMZ -ReferencePortgroup $Referencegroup | Set-VDPortgroup -VlanId $ScriptDMZ -NumPorts 8
}

elseif ($script:Cluster -eq 'NP'){$Referencegroup= Get-VDPortgroup -Name "104-CyberArk"
...
{
$gldatastores = Get-VMHost -Location $TargetCluster|Get-Datastore| where {$_.name -notlike "*Template*"}|Select Name,FreeSpaceGB
...
$InstallDNSZones1 = @'
                    Set-DnsServerForwarder -IPAddress ("10.60.102.11","10.60.102.12") 
                    ...
                    Add-DnsServerPrimaryZone -NetworkID "fdfa:ffff:0:$IP1::/64" -ReplicationScope Forest -DynamicUpdate NonsecureAndSecure;
                    Add-DnsServerPrimaryZone -NetworkID "fdfa:ffff:0:$IP2::/64" -ReplicationScope Forest -DynamicUpdate NonsecureAndSecure;
                    Add-DnsServerPrimaryZone -NetworkID "fdfa:ffff:0:$IP3::/64" -ReplicationScope Forest -DynamicUpdate NonsecureAndSecure;
                    Add-DNSServerConditionalForwarderZone -Name "Itronms.local" -ReplicationScope "Forest" -MasterServers 10.60.102.11,10.60.102.12
'@

$InstallDNSZones = $InstallDNSZones1.Replace('$IP1',$scriptApp).Replace('$IP2',$scriptDB).Replace('$IP3',$scriptDMZ).Replace('$IP',$UIP)
...
$Users2A = @'
           ...
           New-ADUser -Name $IEE -GivenName $IEE -SamAccountName $IEE -Surname "Service" -DisplayName $IEE -UserPrincipalName "$IEE@$Domain.local" -ANPSccountPassword (ConvertTo-SecureString -AsPlainText "Temp1234" -Force) -PasswordNeverExpires 1 -Path "OU=Service_Accounts,OU=Customers,DC=$Domain,DC=local" -Enabled 1
'@
...
Write-Output "UserID:$name `t Password:$NewPassword" `n|FT -AutoSize >>SVCPasswords.txt
...
#Here will write the info to the file, so you can communicate to your users the new password.
Write-Output "UserID:$name `t Password:$NewPassword" `n|FT -AutoSize >>ItronSVCPasswords.txt
...
#Here will write the info to the file, so you can communicate to your users the new password.
Write-Output "UserID:$name `t Password:$NewPassword" `n|FT -AutoSize >>ItronEmployees.txt
...
$GPOLinkA  = @'
             ...
             New-GPO -Name "RDS 2016" -Comment "This is for the RDS Servers License" | new-gplink -Target "DC=$Domain,DC=Local";
             New-GPO -Name "Server Level Access" | new-gplink -Target "DC=$Domain,DC=Local";  
'@
    ...
    $_ | Get-NetIPAddress | Foreach {
        ...
        if ($script:ipv4address) {
            if ($script:ipv4address.split('.')[1] -eq 50 -or 150 -or 250) {
                ...
                $local:dnvlan = 1500+[Int]$script:ipv4address.split('.')[2]
...
$FNDV6 = $FNDV62.Replace('$IP',$UIP).Replace('$vlan',$scriptApp)

$FNDGateway1 = @'
            echo "IPV6_DEFAULTGW=fdfa:ffff:0:$vlan:10:50:$IP:1" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@
$FNDGateway = $FNDGateway1.Replace('$IP',$UIP).Replace('$vlan',$scriptApp)

$FNDDNS1 = @'
            echo "DNS3=fdfa:ffff:0:$vlan:10:50:$IP:11" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@
$FNDDNS = $FNDDNS1.Replace('$IP',$UIP).Replace('$vlan',$scriptApp)

$FNDDNS12 = @'
            echo "DNS4=fdfa:ffff:0:$vlan:10:50:$IP:12" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@
$FNDDNS2 = $FNDDNS12.Replace('$IP',$UIP).Replace('$vlan',$scriptApp)


$FNDDBV62 = @'
          echo "IPV6ADDR=fdfa:ffff:0:$vlan:10:150:$IP:33/64" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@

$FNDDBV6 = $FNDDBV62.Replace('$IP',$UIP).Replace('$vlan',$scriptDB)


$FNDDBGateway1 = @'
            echo "IPV6_DEFAULTGW=fdfa:ffff:0:$vlan:10:150:$IP:1" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@
$FNDDBGateway = $FNDDBGateway1.Replace('$IP',$UIP).Replace('c$vlan',$scriptDB)


$TPSV62 = @'
          echo "IPV6ADDR=fdfa:ffff:0:$vlan:10:250:$IP:17/64" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@

$TPSV6 = $TPSV62.Replace('$IP',$UIP).Replace('$vlan',$scriptDMZ)


$TPSGateway1 = @'
            echo "IPV6_DEFAULTGW=fdfa:ffff:0:$vlan:10:250:$IP:1" >> /etc/sysconfig/network-scripts/ifcfg-ens192
'@
$TPSGateway = $TPSGateway1.Replace('$IP',$UIP).Replace('$vlan',$scriptDMZ)
...
if ($script:Cluster -eq 'P') {New-NetworkAdapter -VM $DomainControllerVMName -NetworkName vLAN-$scriptApp-$script:customer-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false}

elseif ($script:Cluster -eq 'NP') {New-NetworkAdapter -VM $DomainControllerVMName -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false}

if ($script:Cluster -eq 'P') {New-NetworkAdapter -VM $DC02VMName -NetworkName vLAN-$scriptApp-$script:customer-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false}

elseif ($script:Cluster -eq 'NP') {New-NetworkAdapter -VM $DC02VMName -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false}
...
Write-Verbose -Message "Joining $DC02VMName to the domain." -Verbose
...
$DomainTrust = '$strRemoteForest = "Itronms.local"
                $strRemoteAdmin = "melliott"
                $strRemoteAdminPassword = "Sassaroo1254"
...
New-VM -Name $script:customer-$Char-ECC-CA -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-ECC-CA|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-ECC-CA -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-ECC-CA 
...
Get-VM $script:customer-$Char-ECC-CA|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-RSA-CA -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-RSA-CA|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-RSA-CA -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-RSA-CA
...
Get-VM $script:customer-$Char-RSA-CA|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-NPS -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-NPS|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-NPS -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-NPS
...
Get-VM $script:customer-$Char-NPS|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-WIFI-CA -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-WIFI-CA|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-WIFI-CA -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-WIFI-CA
...
Get-VM $script:customer-$Char-WIFI-CA|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-NetworkAdapter -VM $script:customer-$Char-RDS01 -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false
...
New-VM -Name $script:customer-$Char-OW-CM -Template OWCM-41-APP-UCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-OW-CM|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-OW-CM -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-OW-CM
...
Get-VM $script:customer-$Char-OW-CM|Set-VM -MemoryGB 16 -NumCpu 4 -OSCustomizationSpec $script:customer-$Char-OW-CM -Confirm:$false
...
Write-Verbose -Message "Deploying OW-DB Server" -Verbose

$Datastore = Get-Largest

New-VM -Name $script:customer-$Char-OW-DB -Template OW-SQL-Currie -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-OW-DB|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-OW-DB -NetworkName vLAN-$scriptDB-$NP-DB -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-OW-DB
Get-OSCustomizationSpec -Name $script:customer-$Char-OW-DB|Get-OSCustomizationNicMapping|Remove-OSCustomizationNicMapping -confirm:$false
Get-OSCustomizationSpec -Name $script:customer-$Char-OW-DB |New-OSCustomizationNicMapping -IpMode UseStaticIP -IpAddress 10.150.$UIP.31 -SubnetMask 255.255.255.0 -DefaultGateway 10.150.$UIP.1 -Dns 10.50.$UIP.11,10.50.$UIP.12 -Position 1
Get-OSCustomizationSpec -Name $script:customer-$Char-OW-DB |Set-OSCUstomizationSpec -Domain $Dom -DomainCredentials $DomainCredential2 -DnsServer 10.50.$UIP.11,10.50.$UIP.12

Get-VM $script:customer-$Char-OW-DB|Set-VM -MemoryGB 16 -NumCpu 4 -OSCustomizationSpec $script:customer-$Char-OW-DB -Confirm:$false
Get-VM $script:customer-$Char-OW-DB|Set-Annotation -CustomAttribute "Created by" -Value "New VM Script(run as $User)"
$DateTime=Get-Date
Get-VM $script:customer-$Char-OW-DB|Set-Annotation -CustomAttribute "Created on" -Value $DateTime
Get-VM $script:customer-$Char-OW-DB|Set-Annotation -CustomAttribute "Deployment Script Version" -Value $Ver
Get-VM $script:customer-$Char-OW-DB|Set-Annotation -CustomAttribute "Deployment Template" -Value OWOC-SQL-Currie

$spec = New-Object -Type VMware.Vim.VirtualMachineConfigSpec -Property @{"NumCoresPerSocket" = 2}
(get-VM $script:customer-$Char-OW-DB).ExtensionData.ReconfigVM_Task($spec)
Write-Host $script:customer-$Char-OW-DB' Built:'
Write-Host "Booting"
Start-sleep -Seconds 30
# Power On New VM
Start-VM $script:customer-$Char-OW-DB
wait-tools -VM $script:customer-$Char-OW-DB
remove-oscustomizationspec $script:customer-$Char-OW-DB -confirm:$false
...
New-NetworkAdapter -VM $script:customer-$Char-ISM-APP -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false
...
Write-Verbose -Message "Deploying ISM-DB Server" -Verbose

$Datastore = Get-Largest

New-VM -Name $script:customer-$Char-ISM-DB -Template ISM-SQL-Currie -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-ISM-DB|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-ISM-DB -NetworkName vLAN-$scriptDB-$NP-DB -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2012R2Std-GUI|New-OSCustomizationSpec -Name $script:customer-$Char-ISM-DB
Get-OSCustomizationSpec -Name $script:customer-$Char-ISM-DB|Get-OSCustomizationNicMapping|Remove-OSCustomizationNicMapping -confirm:$false
Get-OSCustomizationSpec -Name $script:customer-$Char-ISM-DB |New-OSCustomizationNicMapping -IpMode UseStaticIP -IpAddress 10.150.$UIP.32 -SubnetMask 255.255.255.0 -DefaultGateway 10.150.$UIP.1 -Dns 10.50.$UIP.11,10.50.$UIP.12 -Position 1
Get-OSCustomizationSpec -Name $script:customer-$Char-ISM-DB |Set-OSCUstomizationSpec -Domain $Dom -DomainCredentials $DomainCredential2 -DnsServer 10.50.$UIP.11,10.50.$UIP.12

Get-VM $script:customer-$Char-ISM-DB|Set-VM -MemoryGB 16 -NumCpu 4 -OSCustomizationSpec $script:customer-$Char-ISM-DB -Confirm:$false
Get-VM $script:customer-$Char-ISM-DB|Set-Annotation -CustomAttribute "Created by" -Value "New VM Script(run as $User)"
$DateTime=Get-Date
Get-VM $script:customer-$Char-ISM-DB|Set-Annotation -CustomAttribute "Created on" -Value $DateTime
Get-VM $script:customer-$Char-ISM-DB|Set-Annotation -CustomAttribute "Deployment Script Version" -Value $Ver
Get-VM $script:customer-$Char-ISM-DB|Set-Annotation -CustomAttribute "Deployment Template" -Value ISM-SQL-Currie

$spec = New-Object -Type VMware.Vim.VirtualMachineConfigSpec -Property @{"NumCoresPerSocket" = 2}
(get-VM $script:customer-$Char-ISM-DB).ExtensionData.ReconfigVM_Task($spec)
Write-Host $script:customer-$Char-ISM-DB' Built:'
Write-Host "Booting"
Start-sleep -Seconds 30
# Power On New VM
Start-VM $script:customer-$Char-ISM-DB
wait-tools -VM $script:customer-$Char-ISM-DB
remove-oscustomizationspec $script:customer-$Char-ISM-DB -confirm:$false
...
New-NetworkAdapter -VM $script:customer-$Char-BACKUP -NetworkName vLAN-$scriptDB-$NP-DB -Type Vmxnet3 -StartConnected:$True -Confirm:$false
...
New-VM -Name $script:customer-$Char-FND-APP -Template RHEL7.3-Full-UCS -ResourcePool $RHCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-FND-APP|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-FND-APP -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false
...
New-VM -Name $script:customer-$Char-FND-DB -Template RHEL-7.3-Full-GUI-Oracle12c-UCS -ResourcePool $RHCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-FND-DB|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-FND-DB -NetworkName vLAN-$scriptDB-$NP-DB -Type Vmxnet3 -StartConnected:$True -Confirm:$false
...
New-VM -Name $script:customer-$Char-TPS -Template RHEL7.3-Full-UCS -ResourcePool $RHCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-TPS|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-TPS -NetworkName vLAN-$scriptDMZ-$NP-DMZ -Type Vmxnet3 -StartConnected:$True -Confirm:$false
...
New-NetworkAdapter -VM $script:customer-$Char-FCS-APP -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-FCS-APP
...
Write-Verbose -Message "Deploying FCS-DB Server" -Verbose

$Datastore = Get-Largest

New-VM -Name $script:customer-$Char-FCS-DB -Template FCS-SQL-UCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-FCS-DB|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-FCS-DB -NetworkName vLAN-$scriptDB-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-FCS-DB
Get-OSCustomizationSpec -Name $script:customer-$Char-FCS-DB|Get-OSCustomizationNicMapping|Remove-OSCustomizationNicMapping -confirm:$false
Get-OSCustomizationSpec -Name $script:customer-$Char-FCS-DB |New-OSCustomizationNicMapping -IpMode UseStaticIP -IpAddress 10.150.$UIP.36 -SubnetMask 255.255.255.0 -DefaultGateway 10.150.$UIP.1 -Dns 10.50.$UIP.11,10.50.$UIP.12 -Position 1
Get-OSCustomizationSpec -Name $script:customer-$Char-FCS-DB |Set-OSCUstomizationSpec -Domain $Dom -DomainCredentials $DomainCredential2 -DnsServer 10.50.$UIP.11,10.50.$UIP.12

Get-VM $script:customer-$Char-FCS-DB|Set-VM -MemoryGB 16 -NumCpu 4 -OSCustomizationSpec $script:customer-$Char-FCS-DB -Confirm:$false
Get-VM $script:customer-$Char-FCS-DB|Set-Annotation -CustomAttribute "Created by" -Value "New VM Script(run as $User)"
$DateTime=Get-Date
Get-VM $script:customer-$Char-FCS-DB|Set-Annotation -CustomAttribute "Created on" -Value $DateTime
Get-VM $script:customer-$Char-FCS-DB|Set-Annotation -CustomAttribute "Deployment Script Version" -Value $Ver
Get-VM $script:customer-$Char-FCS-DB|Set-Annotation -CustomAttribute "Deployment Template" -Value FCS-SQL-UCS

$spec = New-Object -Type VMware.Vim.VirtualMachineConfigSpec -Property @{"NumCoresPerSocket" = 2}
(get-VM $script:customer-$Char-FCS-DB).ExtensionData.ReconfigVM_Task($spec)
Write-Host $script:customer-$Char-FCS-DB' Built:'
Write-Host "Booting"
Start-sleep -Seconds 30
# Power On New VM
Start-VM $script:customer-$Char-FCS-DB
wait-tools -VM $script:customer-$Char-FCS-DB
remove-oscustomizationspec $script:customer-$Char-FCS-DB -confirm:$false
...
New-NetworkAdapter -VM $script:customer-$Char-IEE-APP -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-IEE-APP
...
New-NetworkAdapter -VM $script:customer-$Char-IEE-DB -NetworkName vLAN-$scriptDB-$NP-DB -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-IEE-DB
...
New-VM -Name $script:customer-$Char-PM-AGT -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-PM-AGT|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-PM-AGT -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-PM-AGT 
...
Get-VM $script:customer-$Char-PM-AGT|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-PM-HUB -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-PM-HUB|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-PM-HUB -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-PM-HUB 
...
Get-VM $script:customer-$Char-PM-HUB|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-PM-ID -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-PM-ID|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-PM-ID -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-PM-ID 
...
Get-VM $script:customer-$Char-PM-ID|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-PM-MQ -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-PM-MQ|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-PM-MQ -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-PM-MQ 
...
Get-VM $script:customer-$Char-PM-MQ|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-VM -Name $script:customer-$Char-PM-APP -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-PM-APP|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-PM-APP -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-PM-APP 
...
Get-VM $script:customer-$Char-PM-APP|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS
...
New-NetworkAdapter -VM $script:customer-$Char-PM-DB -NetworkName vLAN-$scriptDB-$NP-DB -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-PM-DB 
...
Write-Verbose -Message "Deploying FN-CE Server" -Verbose

$Datastore = Get-Largest

New-VM -Name $script:customer-$Char-FN-CE -Template Win2016Std-DTEUCS -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-FN-CE|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-FN-CE -NetworkName vLAN-$scriptApp-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-FN-CE
Get-OSCustomizationSpec -Name $script:customer-$Char-FN-CE|Get-OSCustomizationNicMapping|Remove-OSCustomizationNicMapping -confirm:$false
Get-OSCustomizationSpec -Name $script:customer-$Char-FN-CE |New-OSCustomizationNicMapping -IpMode UseStaticIP -IpAddress 10.50.$UIP.38 -SubnetMask 255.255.255.0 -DefaultGateway 10.50.$UIP.1 -Dns 10.50.$UIP.11,10.50.$UIP.12 -Position 1
Get-OSCustomizationSpec -Name $script:customer-$Char-FN-CE |Set-OSCUstomizationSpec -Domain $Dom -DomainCredentials $DomainCredential2 -DnsServer 10.50.$UIP.11,10.50.$UIP.12

Get-VM $script:customer-$Char-FN-CE|Set-VM -MemoryGB 4 -NumCpu 2 -OSCustomizationSpec $script:customer-$Char-FN-CE -Confirm:$false
Get-VM $script:customer-$Char-FN-CE|Set-Annotation -CustomAttribute "Created by" -Value "New VM Script(run as $User)"
$DateTime=Get-Date
Get-VM $script:customer-$Char-FN-CE|Set-Annotation -CustomAttribute "Created on" -Value $DateTime
Get-VM $script:customer-$Char-FN-CE|Set-Annotation -CustomAttribute "Deployment Script Version" -Value $Ver
Get-VM $script:customer-$Char-FN-CE|Set-Annotation -CustomAttribute "Deployment Template" -Value Win2016Std-DTEUCS

$spec = New-Object -Type VMware.Vim.VirtualMachineConfigSpec -Property @{"NumCoresPerSocket" = 2}
(get-VM $script:customer-$Char-FN-CE).ExtensionData.ReconfigVM_Task($spec)
Write-Host $script:customer-$Char-FN-CE' Built:'
Write-Host "Booting"
Start-sleep -Seconds 30
# Power On New VM
Start-VM $script:customer-$Char-FN-CE
wait-tools -VM $script:customer-$Char-FN-CE
remove-oscustomizationspec $script:customer-$Char-FN-CE -confirm:$false

Write-Verbose -Message "Deploying FN-DB Server" -Verbose

$Datastore = Get-Largest

New-VM -Name $script:customer-$Char-FN-DB -Template FN-DB-NEW -ResourcePool $TargetCluster -Datastore $Datastore  -Location $Folder
Get-NetworkAdapter $script:customer-$Char-FN-DB|Remove-NetworkAdapter -confirm:$false
New-NetworkAdapter -VM $script:customer-$Char-FN-DB -NetworkName vLAN-$scriptDB-$NP-App -Type Vmxnet3 -StartConnected:$True -Confirm:$false

Get-OSCustomizationSpec -Name Win2016Std-DTE-Baseline|New-OSCustomizationSpec -Name $script:customer-$Char-FN-DB
Get-OSCustomizationSpec -Name $script:customer-$Char-FN-DB|Get-OSCustomizationNicMapping|Remove-OSCustomizationNicMapping -confirm:$false
Get-OSCustomizationSpec -Name $script:customer-$Char-FN-DB |New-OSCustomizationNicMapping -IpMode UseStaticIP -IpAddress 10.150.$UIP.38 -SubnetMask 255.255.255.0 -DefaultGateway 10.150.$UIP.1 -Dns 10.50.$UIP.11,10.50.$UIP.12 -Position 1
Get-OSCustomizationSpec -Name $script:customer-$Char-FN-DB |Set-OSCUstomizationSpec -Domain $Dom -DomainCredentials $DomainCredential2 -DnsServer 10.50.$UIP.11,10.50.$UIP.12

Get-VM $script:customer-$Char-FN-DB|Set-VM -MemoryGB 8 -NumCpu 4 -OSCustomizationSpec $script:customer-$Char-FN-DB -Confirm:$false
Get-VM $script:customer-$Char-FN-DB|Set-Annotation -CustomAttribute "Created by" -Value "New VM Script(run as $User)"
$DateTime=Get-Date
Get-VM $script:customer-$Char-FN-DB|Set-Annotation -CustomAttribute "Created on" -Value $DateTime
Get-VM $script:customer-$Char-FN-DB|Set-Annotation -CustomAttribute "Deployment Script Version" -Value $Ver
Get-VM $script:customer-$Char-FN-DB|Set-Annotation -CustomAttribute "Deployment Template" -Value FN-DB-NEW 
$spec = New-Object -Type VMware.Vim.VirtualMachineConfigSpec -Property @{"NumCoresPerSocket" = 2}
(get-VM $script:customer-$Char-FN-DB).ExtensionData.ReconfigVM_Task($spec)
Write-Host $script:customer-$Char-FN-DB' Built:'
Write-Host "Booting"
Start-sleep -Seconds 30
# Power On New VM
Start-VM $script:customer-$Char-FN-DB
wait-tools -VM $script:customer-$Char-FN-DB
remove-oscustomizationspec $script:customer-$Char-FN-DB -confirm:$false
}
# End of Script